  ...
  return (
    <section className="px-4 sm:px-6 lg:px-16 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-12">{filteredProducts.length === 0 ? <p>Ingen produkter fundet.</p> : filteredProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div>
      </div>
    </section>
  );
}
